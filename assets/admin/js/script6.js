...
function drawAll(error, ageCSV, idCSV, occupations) {
	...
	var colorBar = d3.scale.ordinal()
		.domain(["Sondagem","Dados Funcionais Rodovia","FWD","Tráfego","Cadastro de OAE","Topografia","Sinalização e Dispositivos", "Cadastro de Interseções"])
		.range(["#EFB605", "#E3690B", "#CF003E", "#991C71", "#4F54A8", "#07997E", "#7EB852", "#00CED1"]);	
			...
			if(node.ID in dataById) {
				...
				if(node.ID.lastIndexOf(currentID, 0) === 0  & !hidden) {
					...
					if(drawTitle & showText) {
						...
						//Second the light grey total text, I had inverted this
						...
						chosenContext.textAlign = "center";
						chosenContext.textBaseline = "middle"; 
						chosenContext.fillText("Total "+commaFormat(node.size)+" (km)", nodeX, nodeY + -0.65 * nodeR);
					...
					if (Math.round(barHeight) > 1) {
						...
						for (var j = 0; j < bars.length; j++) {
							...
							// 0.3 the original value
							chosenContext.fillRect(nodeX + -nodeR*0.5, bar.barPiecePosition, bar.width, barHeight);
							chosenContext.fill();
							...
							if(drawLabelText & showText) {
								...
								//chosenContext.fillStyle = "rgba(" + mainTextColor[0] + "," + mainTextColor[1] + ","+ mainTextColor[2] + "," + textAlpha +")";
								chosenContext.fillStyle = "rgba(256,256,256," + textAlpha +")";
	...
	var currentID = "",
		oldID = "",
		kids = ["brasil"]; //needed to check which arced titles to show - only those close to the parent node
