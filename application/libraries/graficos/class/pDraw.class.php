 ...
 class pDraw
  {
   ...
   function drawArrowLabel($X1,$Y1,$Text,$Format="")
    {
     ...
     //para tamanho das setas
     if($Format['Velocity'] != null){
         if($Format['Velocity'] == 0) $Format['Velocity'] = $Format['ZeroFactor']; //funcao estetica para velocidades zeradas
         elseif($Format['Velocity'] < $Format['MinFactor']) $Format['Velocity'] += $Format['FactorSizeArrow'];
         elseif($Format['Velocity'] >= $Format['MinFactor']) $Format['Velocity'] *= $Format['FactorSizeArrow'];         
         $X2 = sin(($Angle+360)*PI/180)*$Format['Velocity']+$X1;
         $Y2 = cos(($Angle+360)*PI/180)*$Format['Velocity']+$Y1;         
         $temp1 = $X1;
         $temp = $Y1;
         $X1 = $X2;
         $X2 = $temp1;
    ...
    //sem tamanho para setas
     if($Format['Velocity'] == null){
         $X2 = sin(($Angle+360)*PI/180)*$Length+$X1;
         $Y2 = cos(($Angle+360)*PI/180)*$Length+$Y1;
         $temp1 = $X1;
         $temp = $Y1;
         $X1 = $X2;
         $X2 = $temp1;
     ...
     if ( $Angle > 0 && $Angle < 180 )
      {
       ...
       if ( $Position == POSITION_TOP )
        $this->drawText($X2,$Y2,$Text,array("R"=>$BorderR,"G"=>$BorderG,"B"=>$BorderB,"Alpha"=>$Alpha,"Align"=>TEXT_ALIGN_BOTTOMRIGHT));
       else
        $this->drawText($X2,$Y2,$Text,array("R"=>$BorderR,"G"=>$BorderG,"B"=>$BorderB,"Alpha"=>$Alpha,"Align"=>TEXT_ALIGN_TOPRIGHT));
      }
     else
      {
       $this->drawLine($X2,$Y2,$X2+$TxtWidth,$Y2,array("R"=>$BorderR,"G"=>$BorderG,"B"=>$BorderB,"Alpha"=>$Alpha,"Ticks"=>$Ticks));
       if ( $Position == POSITION_TOP )
        $this->drawText($X2,$Y2,$Text,array("R"=>$BorderR,"G"=>$BorderG,"B"=>$BorderB,"Alpha"=>$Alpha));
       else
        $this->drawText($X2,$Y2,$Text,array("R"=>$BorderR,"G"=>$BorderG,"B"=>$BorderB,"Alpha"=>$Alpha,"Align"=>TEXT_ALIGN_TOPLEFT));
      }
    }
   ...
   function drawPlotChart($Format=NULL)
    ...
    /**
     * Helps to improve chart with arrows and angles to wind graphs
     * @param string $Format
     * @return multitype:number
     */
    function drawArrowWind($Format=NULL)
    {
        $PlotSize		= isset($Format["PlotSize"]) ? $Format["PlotSize"] : NULL;
        $PlotBorder	    = isset($Format["PlotBorder"]) ? $Format["PlotBorder"] : FALSE;
        $BorderR		= isset($Format["BorderR"]) ? $Format["BorderR"] : 50;
            ...
            if ( $Serie["isDrawable"] == TRUE && $SerieName != $Data["Abscissa"] )
            {
                ...
                if ( $Data["Orientation"] == SCALE_POS_LEFTRIGHT )
                {
                    ...
                    foreach($PosArray as $Key => $Y)
                    {
                        ...
                        $posX[$i] = $X; //ajuste de posicao na grade
                        $posY[$i] = $Y;
//                         $posX[$i] = $X-($DisplayOffset-$SerieHeigth-$BorderSize-$PicOffset);
//                         $posY[$i] = $Y-($DisplayOffset-$SerieWeight-$BorderSize-$PicOffset);
                        ...
                        if ( $Y != VOID )
                        {
                            ...
                            {
                                //plot points
                                $this->drawShape($X,$Y,$Shape,$SerieWeight-1,$PlotBorder,$BorderSize,$R,$G,$B,$Alpha,$BorderR,$BorderG,$BorderB,$BorderAlpha);
                            }
                        }
                        $X = $X + $XStep;
                        $i = $i + 1;
                ...
                else
                {
                    ...
                    foreach($PosArray as $Key => $X)
                    {
                        ...
                        $posX[$i] = $X; //ajuste de posicao na grade
                        $posY[$i] = $Y;
//                         $posX[$i] = $X-($DisplayOffset-$SerieHeigth-$BorderSize-$PicOffset)+9.5;
//                         $posY[$i] = $Y-($DisplayOffset-$SerieWeight-$BorderSize-$PicOffset)+14;
